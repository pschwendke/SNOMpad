#!python3
# plot_ac.py. Utility script for plotting an approach curve.
...
import numpy as np
from trion.analysis.io import load_approach
...
default_channels = ["M1A", "M1P", "O3A", "O4A"]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        usage="usage: plot_ac.py [-h] input [-c CHANNELS [CHANNELS ...]] [-o OUTPUT] [-v]"
        )
    ...
    frame = load_approach(args.input)
    z = frame["Z"]*1E9 # use nm ffs
    z -= np.min(z)
    ...
    for k in args.channels:
        try:
            ...
            logging.warning(f"Channel not in data: {k}")

    if len(channels) == 0:
        choices = list(frame.columns)
        logging.error("No correct channels found. Possible choices: "+", ".join(choices))
        sys.exit(1)
